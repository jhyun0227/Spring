			...
			<c:forEach var="board" items="${list }">
				<tr>
					...
					<c:if test="${board.del != 'y' }">
						...
						<a class="btn btn-info btn-sm" href="view.do?num=${board.num }&pageNum=${pb.currentPage}">${board.subject }</a>
	...
	<div align="center">
		<ul class="pagination">
			...
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<!-- 첫 페이지로 -->
				<li>
					<a href="list.do?pageNum=1&search=${board.search }&keyword=${board.keyword}"><span class="glyphicon glyphicon-backward"></span></a>
				</li>
				<!-- 전 블록으로 -->
				<li>
					<a href="list.do?pageNum=${pb.startPage-1 }&search=${board.search }&keyword=${board.keyword}"><span class="glyphicon glyphicon-triangle-left"></span></a>
				</li>
			</c:if>
			...
			<c:if test="${pb.endPage < pb.totalPage }">
				<!-- 다음 블록으로 -->
				<li>
					<a href="list.do?pageNum=${pb.endPage + 1 }&search=${board.search }&keyword=${board.keyword}"><span class="glyphicon glyphicon-triangle-right"></span></a>
				</li>
				<!-- 마지막 페이지로 -->
				<li>
					<a href="list.do?pageNum=${pb.totalPage }&search=${board.search }&keyword=${board.keyword}"><span class="glyphicon glyphicon-forward"></span></a>
				</li>
			</c:if>
		</ul>
	</div>
