...
public class MemberDaoImpl implements MemberDao {
	...
	public int insert(Member member) {
		// update는 입력 수정 삭제 사용
		int result = jt.update("insert into member3 values(?,?,?,?,sysdate)", member.getId(), member.getEmail(),
	...
	public int update(Member member) {
		int result = jt.update("update member3 set email=?, password=?, name=? where id=?", member.getEmail(),
				member.getPassword(), member.getName(), member.getId());
		return result;
	}
